    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header header-elements-inline bg-danger">
                <h6 class="card-title font-weight-bold">SIFAT AFEKTIF</h6>
                {!! Qs::getPanelOptions() !!}
            </div>
                ...
                <form class="ajax-update" method="post" action="{{ route('marks.skills_update', ['AF', $exr->id]) }}">
                    ...
                    @foreach ($skills->where('skill_type', 'AF') as $af)
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select data-placeholder="Pilih" name="af[]" id="af"
                                    class="form-control select">
                                    <option value=""></option>
                                    @for ($i = 1; $i <= 5; $i++)
                                        <option
                                            {{ $exr->af && explode(',', $exr->af)[$loop->index] == $i ? 'selected' : '' }}
                                            value="{{ $i }}">{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
    ...
    <div class="col-md-6">
        <div class="card">
            <div class="card-header header-elements-inline bg-success">
                <h6 class="card-title font-weight-bold">KETERAMPILAN PSIKOMOTOR</h6>
                {!! Qs::getPanelOptions() !!}
            </div>
                ...
                <form class="ajax-update" method="post" action="{{ route('marks.skills_update', ['PS', $exr->id]) }}">
                    ...
                    @foreach ($skills->where('skill_type', 'PS') as $ps)
                        <div class="form-group row">
                            ...
                            <div class="col-lg-6">
                                <select data-placeholder="Pilih" name="ps[]" id="ps"
                                    class="form-control select">
                                    <option value=""></option>
                                    @for ($i = 1; $i <= 5; $i++)
                                        <option
                                            {{ $exr->ps && explode(',', $exr->ps)[$loop->index] == $i ? 'selected' : '' }}
                                            value="{{ $i }}">{{ $i }}</option>
                                    @endfor
                                </select>
                            </div>
