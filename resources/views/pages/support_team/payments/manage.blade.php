...
@section('page_title', 'Pembayaran Siswa')
@section('content')
    <div class="card">
        <div class="card-header header-elements-inline">
            <h5 class="card-title"><i class="icon-cash2 mr-2"></i> Pembayaran Siswa</h5>
            {!! Qs::getPanelOptions() !!}
        </div>
                    ...
                    <tbody>
                        @foreach ($students as $s)
                            <tr>
                                ...
                                <td>
                                    <div class="dropdown">
                                        <a href="#" class=" btn btn-danger" data-toggle="dropdown"> Kelola Pembayaran
                                            <i class="icon-arrow-down5"></i>
                                        </a>
                                        <div class="dropdown-menu dropdown-menu-left">
                                            <a href="{{ route('payments.invoice', [Qs::hash($s->user_id)]) }}"
                                                class="dropdown-item">Semua Pembayaran</a>
                                            @foreach (Pay::getYears($s->user_id) as $py)
                                                @if ($py)
                                                    <a href="{{ route('payments.invoice', [Qs::hash($s->user_id), $py]) }}"
