...
@section('content')
    ...
    <div class="card">
        <div class="card-header header-elements-inline">
            <h6 class="card-title">Kelola Jadwal</h6>
            {!! Qs::getPanelOptions() !!}
        </div>
        ...
        <div class="card-body">
            <ul class="nav nav-tabs nav-tabs-highlight">
                @if (Qs::userIsTeamSA())
                    <li class="nav-item"><a href="#add-tt" class="nav-link active" data-toggle="tab">Buat Jadwal</a></li>
                @endif
                <li class="nav-item dropdown">
                    <a href="#" class="nav-link dropdown-toggle" data-toggle="dropdown">Tampilkan Jadwal</a>
                    ...
                    <div class="tab-pane fade show active" id="add-tt">
                        <div class="col-md-8">
                            <form class="ajax-store" method="post" action="{{ route('ttr.store') }}">
                                ...
                                <div class="form-group row">
                                    <label for="my_class_id" class="col-lg-3 col-form-label font-weight-semibold">Kelas
                                        <span class="text-danger">*</span></label>
                                    <div class="col-lg-9">
                                        <select required data-placeholder="Pilih Kelas" class="form-control select"
                                            name="my_class_id" id="my_class_id">
                                            @foreach ($my_classes as $mc)
                                                <option {{ old('my_class_id') == $mc->id ? 'selected' : '' }}
                                ...
                                <div class="form-group row">
                                    <label for="exam_id" class="col-lg-3 col-form-label font-weight-semibold">Tipe (Kelas
                                        atau Ujian)</label>
                                    <div class="col-lg-9">
                                        <select class="select form-control" name="exam_id" id="exam_id">
                                            <option value="">Jadwal Kelas</option>
                                            @foreach ($exams as $ex)
                                                <option {{ old('exam_id') == $ex->id ? 'selected' : '' }}
                                ...
                                <div class="text-right">
                                    <button id="ajax-btn" type="submit" class="btn btn-primary">Kirim formulir <i
                            ...
                            <tbody>
                                @foreach ($tt_records->where('my_class_id', $mc->id) as $ttr)
                                    <tr>
                                        ...
                                        <td>{{ $ttr->exam_id ? $ttr->exam->name : 'Jadwal Kelas' }}
                                        <td>{{ $ttr->year }}</td>
                                        <td class="text-center">
                                            <div class="list-icons">
                                                <div class="dropdown">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-right">
                                                        {{-- Lihat --}}
                                                        <a href="{{ route('ttr.show', $ttr->id) }}"
                                                            class="dropdown-item"><i class="icon-eye"></i> Lihat</a>
                                                        ...
                                                        @if (Qs::userIsSuperAdmin())
                                                            <a id="{{ $ttr->id }}" onclick="confirmDelete(this.id)"
